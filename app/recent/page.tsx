...
export default function RecentPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const data = await getRecentlyUpdatedManga(limit)
        setManga(data || [])
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      // For now, we'll just fetch more recent manga
      const data = await getRecentlyUpdatedManga(limit)
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="mb-6 text-3xl font-bold">Recently Updated Manga</h1>
      ...
      {manga.length > 0 ? (
        ...
        <div className="rounded-lg border p-8 text-center">
          ...
          <p className="text-muted-foreground">There are no recently updated manga available at the moment.</p>
        </div>
      )}
